...
func BenchmarkRegexpMatchString(b *testing.B) {
	...
	needle := getStringNeedle()
	for n := 0; n < b.N; n++ {
		_ = regexpMatchString(needle, haystack)
	}
}
...
func BenchmarkRegexpMatchCompiled(b *testing.B) {
	...
	needle := getStringNeedle()
	for n := 0; n < b.N; n++ {
		_ = regexpMatchCompiled(needle, haystack)
	}
}
